...
(function ($) {
    ...
    $.fn.ColorPicker = function () {
            ...
            go = {
                convert:     {
                    ...
                    hsv: function (hue_only, hue_bar) {
                        ...
                        var sat = hue_only ? 1 : data.color.hsv[1] / 100,
                    ...
                    rgb: function () {
                        ...
                        for (var i = 0; i < 3; i++) {
                            var color_code = data.color.rgb[i].toString(16);
                ...
                open_picker: function () {
                    ...
                    var get_hue = go.convert.hsv(true),
                        dialog = '';
                ...
                set_position: function (obj, off, y, x) {
                    ...
                    var css = {top: y < 0 ? 0 : y > data.size ? data.size : y};
                ...
                set_color: function (move_position) {
                    $.each(elem.value, function (index, input) {
                        var color_code = index < 3 ? data.color.rgb[index] : data.color.hsv[index - 3];
                        input.val(Math.round(color_code));
                    });

                    var hex = '#' + data.color.hex[0] + data.color.hex[1] + data.color.hex[2],
        ...
        this.prev().on({
            input: function () {
                var obj = $(this),
                    val = ('#' + obj.val().replace(/[^0-9A-F]+/gi, '')).substr(0, 7);

                obj.val(val);

                var valid = /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(val); // проверка на соответствие выражению
            ...
            blur: function () {
                ...
                if (val.length == 4) {
                    obj.val(val.replace(/([0-9A-F])([0-9A-F])([0-9A-F])/i, '$1$1$2$2$3$3'));
                } else {
                    obj.val('#' + data.color.hex[0] + data.color.hex[1] + data.color.hex[2]);
                }
            },
